 ...
 */
const SYSTEM = `You tag 3D-shape-generation papers for a curated archive.

Return STRICT JSON matching this exact schema:

{
  "is_shape_paper": boolean,         // true iff the paper's CENTRAL contribution is GENERATING 3D shape/geometry — synthesizing new 3D objects/scenes (mesh, point cloud, voxel, SDF, implicit field, gaussian, triplane) via diffusion / autoregressive / GAN / VAE / flow / feed-forward / score-distillation. Includes text-to-3D, image-to-3D, unconditional & conditional 3D generation, and generative shape editing/completion. Reject: pure 2D image/video generation with no 3D output, deterministic single-solution reconstruction or SLAM/MVS, novel-view synthesis (NeRF/3DGS) without a generative model, depth/normal estimation, segmentation/classification/retrieval, pose estimation, NLP, robotics.
...
Rules:
- Accept ONLY when the MAIN target is generating 3D shape/geometry. A paper that reconstructs a single 3D model deterministically (no generative/probabilistic model, no sampling) is NOT shape generation — reject it.
